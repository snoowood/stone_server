...
| 11 | POST | `/achievement/unlock` | JWT | 업적 해제 | `achievement_id` | 200 |
| 12 | GET | `/achievement/list` | JWT | 업적 목록 | — | 200 |
| 13 | POST | `/auth/dev` | Public(dev) | UUID 무비번 로그인 | `uuid` | 200 |
| 14 | GET | `/internal/dev-token` | dev | 로드테스트 토큰 | `?steam_id` | 200 |
...
**`inventory[]`**: `item_id`(string), `rarity`(string), `count`(int), `acquired_at`(datetime)
...
### 5.1 POST /achievement/unlock — 업적 해제
...
### 5.2 GET /achievement/list — 업적 목록
...
- **에러 바디**: 항상 `{"error","code"}`. 클라는 `(HttpStatus, Code)` 조합으로 분기.
